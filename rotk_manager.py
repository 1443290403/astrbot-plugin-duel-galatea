...
class RotKManager:
    ...
    def _clear_image_cache(self):
        if os.path.exists(self.img_dir):
            for filename in os.listdir(self.img_dir):
                try:
                    os.unlink(os.path.join(self.img_dir, filename))
                except:
                    pass
    ...
    async def _fetch_article_content_images(self, session, article_url):
        ...
        if end_idx == -1:
            end_idx = content.find('class="sharedaddy"', start_idx)
        if end_idx == -1:
            end_idx = content.find('id="comments"', start_idx)
        if end_idx == -1:
            end_idx = len(content)
        ...
        for img in raw_imgs:
            if "wp-content/uploads" not in img:
                continue
            if any(kw in img.lower() for kw in ignore):
                continue
        ...
        async with aiohttp.ClientSession(trust_env=True, headers=self.headers) as session:
            ...
            for raw_article in articles[1:]:
                title_match = re.search(
                    r"entry-title.*?<a[^>]+>([^<]+)</a>", raw_article, re.DOTALL
                )
                if not title_match:
                    continue
                ...
                if "metagame report" not in title.lower():
                    continue
